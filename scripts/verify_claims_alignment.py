...
- A1 反向特征差集 (L8-0 反向断言, 唯一出处 implementation.md L8-0):
  说明书三节出现、而权利要求书不存在的实体名词 (带"所述/预设"特征指代形态
  的专名). 语义边界靠正则近似, 判为 **suspect** (供 auditor 复核).
- A2 权要内撞名 (claims-requirements.md L1-1): 同一实体名词在权要内被多处赋值为不同来源
  (既"提取...得到 X"又"将...作为 X"). 语义, 判为 **suspect**.
- A3 步骤集差集 (implementation.md L8-1): 权要每条分句的末端产物名是否在三节被
...
# -----------------------------------------------------------------------------
# 实体名词 / 产物名抽取 (正则近似; 实体名词定义见 implementation.md L8-0)
# -----------------------------------------------------------------------------
