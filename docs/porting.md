...
```
core          内核开发主线: references/ scripts/ assets/ agents/*.md rules.md docs/
              （不含各宿主专属的 SKILL.md 编排与 openai.yaml）
 ├── claude   = core + SKILL.md(Claude 范式)
 └── codex    = core + SKILL.md(Codex 范式) + agents/openai.yaml
```

> `SKILL.md` 在两个适配分支同名但内容不同，故它是**适配层**、不进 core 共享。core→适配分支的 merge 若碰到 `SKILL.md`，以适配分支自身版本为准（`git checkout --ours SKILL.md`）；规则/脚本改动不碰 `SKILL.md`，正常 merge 无冲突。
