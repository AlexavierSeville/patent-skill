...
- **附图退出审查**:用户自行生图画图。`figures.md` 不进任何规则包;原 rule-auditor 语义必审清单中"权要 1 vs 图 1 节点""附图说明 vs 附图设计图数"两项删除;完整性清单中"附图设计节(L9)"降级为存在性检查;`scoring.md` 中"L7-1 与 L9 一致"降级为 L7 自身格式检查。**(2026-07 更新:附图改由 `render_patent_figure.py` 从权要 1 自动生成、`insert_figures_docx.py` 注入 DOCX,存在性检查对象改为 `docs/figures/figure-1.png`,"权要 1 vs 图 1 节点"一致性由脚本构造保证;见 `figures.md` L9。)**
- **返修阶段按改动块定向调 auditor(2026-07 修订；`stage` 仍传 `claims-draft`/`full-draft` 稿件类型，返修态由 `reaudit_context` 标识，不存在 `*-revision` 取值)**:早期决策为"返修不调 auditor,沿用现有边界不扩大",经 X2607008 全文三稿复盘证伪——返修改动量大、且会引入一稿审查未见过的新文本(如新写的 AI 自辩句、跨技术体系术语),裸奔无语义兜底,一次暴露 19 条批注。现改为:返修对重灾区块(L1/L6/L8)做补写/改写/扩写时,以增量复核模式(`reaudit_context`=本轮批注+改动块清单)定向调用改动块对应的 auditor 路,合并落盘 `docs/审查报告-<稿次>.md`,未过闸不注入;纯格式/纯编号迁移可豁免语义审查、**纯删除不豁免**（至少调 global-auditor 核联动完整性）。执行细则唯一出处 `SKILL.md` 返修工作流。**(2026-08 再修订:多路语义复核的强制范围仅限首次生成稿——全文一稿三路、权要一稿两路强制;返修阶段 auditor 由"必过"改为"先询问用户",用户同意才以增量复核模式调用,拒绝则跳过并在完工报告注明;机械脚本闸门与 md 基准同步不受放松,所有返修仍强制。)**
- **强制范围边界(2026-08 新增)**:全文一稿三路、权要一稿两路为唯一强制场景;返修阶段(全文二稿/三稿、权要二稿/三稿)auditor 语义复核一律先询问用户是否调用,不强制。判断口诀:首次生成稿必审,返修稿问用户。
...
`subagent` 分支退役删除(内容与 claude 分支完全重复)。core 开发直接在 **claude 分支**(权威)进行,codex 分支接收 merge(SKILL.md 冲突以 codex 版本为准)。（`docs/porting.md` 已随 codex 分支一并删除，见 §7 沿革说明）
