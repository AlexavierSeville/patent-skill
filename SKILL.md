...
1. **读返修意见**：先按 `rules.md` 阶段读取表读取 `references/rules/revision.md` 和 `references/rules/docx-template.md`，并根据批注涉及内容读取 `references/rules/claims.md`、`references/rules/full-draft.md` 或 `references/rules/figures.md`；再调用官方 `document-skills:docx`（即 docx）skill，由其提取批注稿 `word/comments.xml` 的批注 + `word/document.xml` 的 tracked changes，逐条列出老板要求。
   - **冻结块只读体检（1级，仅全文返修且权要已单独定稿时做）**：先按 `full-draft.md`「阶段映射与权要冻结」节的适用边界判本案属哪个工作流——案件文件夹根目录有独立权要稿 DOCX（含 `审核N-…权要N稿…docx`）→ 权要已冻结，本轮为全文返修时须做体检（规则按全文一稿 step 1 的三段 `sed -n` 定向提取 L1-1/L2-1/L3-1，只读 1 级块）；违反项不擅改，记入完工报告「提请老板确认清单」，但**本轮批注恰好命中同一项时按批注直接改**。无独立权要稿 DOCX → 属直写全文稿工作流，权要与全文同轮返修、自始不冻结，**不做体检**，批注要求改权要即直接改。权要返修（`claims revision`）本身以权要为工作对象，亦不做体检。
...
1. 先按 `rules.md` 阶段读取表读取 `references/rules/full-draft.md`、`references/rules/figures.md`、`references/rules/docx-template.md` 与 `references/rules/scoring.md`（后者主 agent 合并计分口径所需）；**全文阶段不整篇读 `references/rules/claims.md`**（权要已冻结，全文只补说明书，术语一致与权要对应已由 `global.md G4`、`full-draft.md L6` 覆盖），仅两种情形按需**定向提取**：①本步的冻结块只读体检；②返修批注涉及权要联动。从最新已审权要 DOCX 开始，不从空白模板起稿；若候选不唯一，必须先请用户确认使用哪一份权要 DOCX。并从该 DOCX 提取权利要求书全文作为全文稿唯一权要基准（**DOCX 为权威源**）：提取后与 `docs/权要稿.md` 的权利要求书比对，不一致时以 DOCX 为准**回写更新 `docs/权要稿.md`**，使后续闸门与三路 auditor 使用的 `claims_md_path` 基准与 DOCX 保证一致；一致则直接使用。若 `docs/全文稿.md` 已存在且其权要内容或撰写时间早于最新已审权要，必须先按 `full-draft.md` L8-0 对照新权要重构受影响章节，禁止直接复用旧稿注入。
   - **冻结块只读体检（1级，开工前置，规则唯一出处 `full-draft.md`「阶段映射与权要冻结」节）**：本工作流（权要已单独定稿冻结）必做。对上一句刚确定的权要基准与稿内技术领域、背景技术做一遍只读扫描，规则用 `sed -n '/^### L1-1 /,/^### L1-2 /p'`、`sed -n '/^### L2-1 /,/^### L2-2 /p'`、`sed -n '/^### L3-1 /,/^### L3-2 /p'` 三段定向提取（**只取 1 级块**，不读 2 级与自检清单；提取为空即按输入不完整处理，不得静默跳过）。发现违反项**不擅改冻结块**，逐项记入完工报告「提请老板确认清单」（含所在权要条/块、违反的规则编号、原文摘录）。**直写全文稿工作流不执行本项**（权要与全文同轮产出、自始不冻结，判据见该节适用边界）。
2. 除非用户要求改动，保留已审权要不变。
...
- **校验清单**（权要一稿/全文一稿交付时必附）：**直接从合并落盘的 `docs/审查报告-*.md` 与验收结论转录，逐规则编号打勾，不重新校验**。用 emoji 标记：✅ 通过、❌ 失败（注明原因）、➖ 不适用（注明理由）。按块分组列出，格式如下：
  ...
  ✅ L1-1 权 1 ≤400 字
  ✅ L1-1 从权依附关系两轨平权（2 级质量项；轨 A 逐句对应型全拉平与轨 B 分支＋局部链条型均合格，多元化门槛仅轨 B 适用，见 `claims.md` L1-1）
  ✅ L6-1 发明内容逐条权要覆盖
...
- 是否有未解决的需要用户澄清的事项
- **提请老板确认清单**（返修稿与全文稿交付必附；无项写「无」）：把"已按批注/亲笔执行、但与规则或可实施性存在冲突"以及"发现了但按规矩不擅改"的项集中列出，使冲突可见而不是被静默吞掉（对外可见通道，判据唯一出处 `rules.md`「冲突处理」决策树）。三类来源必须都过一遍：①**批注与 1 级硬规则或可实施性冲突**——默认执行批注，同时列此清单；②**冻结块只读体检**发现的违反项（`full-draft.md`「阶段映射与权要冻结」节）——不擅改冻结块，逐项列出；③**审核人亲笔重写的文本与硬规则字面冲突**（`revision.md` A4-1-核心）——一字不改保留，列出。每项写全四段：位置（块/权要条/段）+ 涉及规则编号 + 冲突或违反内容摘录 + 已采取的处置（执行批注／保留亲笔／未改冻结块）。**任何一类有项而清单未列 = 未完成交付。**
