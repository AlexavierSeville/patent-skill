...
在撰写或修订前，必须先阅读 `rules.md` 和 `references/rules/global.md`。然后根据 `rules.md` 的阶段读取表，仅读取当前任务对应的阶段规则文件。不要默认读取无关阶段规则；例如权要一稿不默认读取 `references/rules/implementation.md`、`references/rules/figures.md`，除非用户明确要求全文、摘要、附图或具体实施方式。
...
权要一稿 step 7 与全文一稿 step 8 的闸门都按本节执行，各 step 只写差异（脚本参数、auditor 路名、传入字段）。**本节多路语义复核的强制范围仅限首次生成稿**——全文一稿三路（content/impl/global-auditor）、权要一稿两路（claims/global-auditor）为强制；返修阶段（全文二稿/三稿、权要二稿/三稿）的 auditor 语义复核**不强制**，机械脚本闸门照常强制，auditor 是否调用按 `SKILL.md` 返修工作流 step 4 询问用户：
...
- **降级兜底**：任一路两次失败（无契约结构或报告"输入不完整"）时，仅该路降级为主 agent 按其契约自查并在完工报告标注"<路名> 未生效"，其余路照常生效，但脚本闸门必须通过。整次编排的降级链为**按次计数、自动切换**：第 1 次尝试 `Workflow` 工具编排（一次编完全部 auditor），失败（工具不可用/脚本报错/全部路无返回）即切第 2 次尝试 **Agent 工具逐路并行派发**（每路一个 subagent，prompt 与输入字段同调用机制条，保留物理隔离与并行独立性）；第 2 次失败再切回 `Workflow` 第 3 次重试；**Workflow 与 Agent 派发共享一个计数器，累计 3 次失败即自动切换为主 agent 串行审查**——主 agent 按各路契约逐路串行自查（每轮只带一路规则包、一轮一审不并行），完工报告标注"主 agent 串行审查（累计 3 次编排失败自动切换）"。切换后本审查流程内锁死串行，不再回退 Workflow/Agent；回修重审轮是新流程，重新从 `Workflow` 编排开始尝试。任何降级级别下脚本闸门都必须通过；降级不得静默——完工报告校验清单首行必须写明本次审查的实际执行方式（Workflow 编排 / Agent 派发 / 主 agent 串行审查）。

## 权要一稿

`权要一稿`：

1. 先按 `rules.md` 阶段读取表读取 `references/rules/claims-requirements.md`、`references/rules/docx-template.md` 与 `references/rules/scoring.md`（后者主 agent 合并计分口径所需，可按需只取「计分方法」节）（此阶段只读其 G8-0/G8-0b 及 md 层可判定条目；G8-1 XML 骨架细节留到 DOCX 执行层 step 8-12 再读）。确认案件文件夹下已有 `docs/` 子目录（无则创建），并把交底书 DOCX 放入 `docs/`。先调用官方 `document-skills:docx`（即 docx）skill 满足执行层门槛（**不用其向上下文回读交底书正文**），随后直接执行 `scripts/disclosure_docx_to_md.py --input <案件文件夹>/docs/交底书.docx --output <案件文件夹>/docs/交底书.md` 完成确定性转换；仅当脚本转换失败或批注锚点异常时，才用 `docx` skill 读原 DOCX 定点排查。交底书全文只在 step 2 深读 `交底书.md` 时进入上下文一次。
...
6. 在 `权要稿.md` 中写完整的权要集、技术领域、背景技术。权要数量、保护主题组合（方法权/计算机设备式系统权/存储介质权）、权要 1 字数、分号断行、从权粒度和依附关系的唯一出处是 `references/rules/claims-requirements.md` 及其指向的 `references/cases/claims-format-standard.md`。
7. 写入 DOCX 之前，主 agent 只自查**脚本与 auditor 均不覆盖的项**：未误写权要一稿以外的章节（摘要等属全文一稿，见 `abstract-figures.md` L4）。机械项交阶段 A 脚本终判、语义项交阶段 B auditor 专审，主 agent 不预演 `claims-requirements.md` 自检清单。**随后按两阶段闸门流程审查**：
...
12. 完成后由 `docx` 执行层做通用 DOCX 验证，再由 `patent` 按 `docx-template.md` G8-0（分节3、分节4 内容落位）、G8-0b（发明名称段六属性由 `verify_docx_skeleton.py` 机械校验、五个章节标题格式人工核对）、G8-1（骨架、可见性、案例性术语清理、权要1字数、分号断行）逐项验收。验收通过后，把 `docs/权要稿.md` 复制为 `docs/history/权要1稿.md`（快照留档，命名见 G1）。
...
1. **读返修意见**：先按 `rules.md` 阶段读取表返修行读取必读核心（`references/rules/revision.md`、`references/rules/docx-template.md`、`references/rules/global.md`），再按 `revision.md`「返修节级读取表」按批注命中内容块 `sed -n` 定向提取对应规则节——命中 L8 提 `implementation.md` L8 节＋`global.md` G6-1 节，命中权要提 `claims-requirements.md` L1 节，命中发明内容提 `invention-content.md` L6 节，命中摘要/附图说明提 `abstract-figures.md` 对应节、命中附图提 `figures.md` L9 节（**不整篇读取阶段规则文件**；批注命中权要条文或 L8 步骤改动时，五处镜像块对应节按读取表自动并入）；再调用官方 `document-skills:docx`（即 docx）skill，由其提取批注稿 `word/comments.xml` 的批注 + `word/document.xml` 的 tracked changes，逐条列出老板要求。
   - **冻结块只读体检（1级，仅全文返修且权要已单独定稿时做）**：先按 `implementation.md`「阶段映射与权要冻结」节的适用边界判本案属哪个工作流——案件文件夹根目录有独立权要稿 DOCX（含 `审核N-…权要N稿…docx`）→ 权要已冻结，本轮为全文返修时须做体检（规则按全文一稿 step 1 的三段 `sed -n` 定向提取 L1-1/L2-1/L3-1，只读 1 级块）；违反项不擅改，记入完工报告「提请老板确认清单」，但**本轮批注恰好命中同一项时按批注直接改**。无独立权要稿 DOCX → 属直写全文稿工作流，权要与全文同轮返修、自始不冻结，**不做体检**，批注要求改权要即直接改。权要返修（`claims revision`）本身以权要为工作对象，亦不做体检。
...
4. **返修定向审查闸门**（重灾区块补写/改写时必过；其中机械脚本闸门强制、auditor 语义复核询问用户；执行细则同「审查闸门通用规则」，只按本轮改动块定向收窄）：本轮对 L1/L6/L8 做了补写/改写/扩写（非纯格式、纯编号迁移）时，注入 DOCX 前必须先过闸——**机械脚本闸门强制**：返修入口先跑守卫——`timestamp_guard.py --case` 两类工作流均跑；`fingerprint_claims.py --check` 仅在**权要已冻结的分离式工作流**跑（挡"拿旧 md 注入新权要"），直写全文稿工作流下权要与全文同稿同轮推进、老板批注要求改权要属正常动作，该守卫会每轮必然硬停，故不跑。随后**无条件跑两套共 5 个脚本，不判断"本轮批注有没有碰权要"**——`权要稿.md` 上跑 `check_hard_rules.py --stage claims-draft` + `check_cross_block.py --stage claims-draft`，`全文稿.md` 上跑 `check_hard_rules.py --stage full-draft --claims-md <案件文件夹>/docs/权要稿.md --invention-name "<正式题名全称>" --draft-round <本轮稿次>` + `check_cross_block.py --stage full-draft` + `verify_claims_alignment.py`（反向特征差集与步骤集差集正是镜像失同步的机械探测器）。**权要那两个脚本不可省**：L1 机械规则只在 md 含权利要求书章节或 `--stage claims-draft` 时执行，而 `全文稿.md` 不含该章节，省掉即等于 L1 全套静默跳过。任一 FAIL 定点回修后重跑，不得跳过；**全文返修的 `check_hard_rules.py` 必带 `--claims-md` 与 `--draft-round <本轮稿次>`**（本轮产出第几稿就传几，如出全文3稿传 `--draft-round 3`），据以校验 L8-1 稿次字数区间——2/3/4 稿上限 20000（Word「字数」口径、`全文稿.md`＋`权要稿.md` 合并计）。**返修尤须盯字数**：返修批注多为"补 how／补标定／补示例"，字数只增不减，最易在此破线；超限时按 L8-1 精简冗余展开、合并同源段落，不得删可实施细节。**字数只认接受修订后的口径**——留痕稿在"显示标记"状态下 Word 状态栏把删除线文本一并计入，读数可虚高一倍，核字数须切"无标记"视图或另存接受修订的干净稿；**auditor 语义复核不强制、先询问用户**：机械脚本全 PASS 后，向用户确认本轮是否调用多路审核（全文返修即三路 content/impl/global-auditor、权要返修即两路 claims/global-auditor，均以增量复核模式执行）。获用户同意才调用——以增量复核模式（`reaudit_context` = 本轮批注清单 + 改动块清单；**`stage` 照传所属稿件类型**——改 L1/L2/L3 传 `claims-draft`、改 L4–L8 传 `full-draft`，`stage` 表稿件类型不表稿次，全系统只有这两值，不得自造 `*-revision`）调用改动块对应的 auditor 路（改 L1 调 claims-auditor、改 L6 调 content-auditor、改 L8 调 impl-auditor；术语或短块联动一律加 global-auditor），按「审查闸门通用规则」合并落盘 `docs/审查报告-<稿次>.md`；用户拒绝调用则跳过 auditor 语义复核，直接进入 step 5 留痕注入，完工报告校验清单注明"本轮未调 auditor（用户选择不调用）"。纯留痕格式修订、纯编号迁移可豁免本闸的语义审查；**纯删除同需先询问用户**——删除会牵动镜像块、引用关系与依赖链，默认建议至少以增量复核模式调 global-auditor 核查联动完整性，最终是否调用以用户答复为准；所有返修均须跑本阶段全部脚本闸门。
...
- `w:ins` 内 run 复制原 run 的 `<w:rPr>`（保字体字号）；`w:id` 取现有最大值以上的未占用值；整段删除时在 `<w:pPr><w:rPr>` 加 `<w:del/>`。
- **`w:date` 一稿一序列、条条不同（去 AI 痕迹，唯一出处本条）**：本轮以 `<用户指定署名>` 名义新增的**全部**痕迹与回复批注（`w:ins` + `w:del` + `comments.xml` 回复，含段落标记 `<w:ins/>`/`<w:del/>`）合并计数为 N，注入前**一次性**执行 `python3 scripts/gen_comment_timestamps.py --count N` 取得一条单调递增、相邻间隔 1-4 分钟随机的时间戳序列，按注入顺序逐条取用；同一处改写的 `w:ins` 与配对 `w:del` 视作一条、共用同一时间戳（同一动作两面，分开反而失真）。**禁止**全轮共用一个 `date +...` 取值——审阅窗格里全部痕迹同分同秒是最显眼的机器批量生成特征。老板原有痕迹的 `w:date` 一律不动。
...
1. 先按 `rules.md` 阶段读取表读取 `references/rules/implementation.md`、`references/rules/figures.md`、`references/rules/docx-template.md` 与 `references/rules/scoring.md`（后者主 agent 合并计分口径所需）；**全文阶段不整篇读 `references/rules/claims-requirements.md`**（权要已冻结，全文只补说明书，术语一致与权要对应已由 `global.md G4`、`invention-content.md L6` 覆盖），仅两种情形按需**定向提取**：①本步的冻结块只读体检；②返修批注涉及权要联动。从最新已审权要 DOCX 开始，不从空白模板起稿；若候选不唯一，必须先请用户确认使用哪一份权要 DOCX。并从该 DOCX 提取权利要求书全文作为全文稿唯一权要基准（**DOCX 为权威源**）：提取后与 `docs/权要稿.md` 的权利要求书比对，不一致时以 DOCX 为准**回写更新 `docs/权要稿.md`**，使后续闸门与三路 auditor 使用的 `claims_md_path` 基准与 DOCX 保证一致；一致则直接使用。若 `docs/全文稿.md` 已存在且其权要内容或撰写时间早于最新已审权要，必须先按 `implementation.md` L8-0 对照新权要重构受影响章节，禁止直接复用旧稿注入。
   - **冻结块只读体检（1级，开工前置，规则唯一出处 `implementation.md`「阶段映射与权要冻结」节）**：本工作流（权要已单独定稿冻结）必做。对上一句刚确定的权要基准与稿内技术领域、背景技术做一遍只读扫描，规则用 `sed -n '/^### L1-1 /,/^### L1-2 /p' claims-requirements.md`、`sed -n '/^### L2-1 /,/^### L2-2 /p' claims-field-background.md`、`sed -n '/^### L3-1 /,/^### L3-2 /p' claims-field-background.md` 三段定向提取（**只取 1 级块**，不读 2 级与自检清单；提取为空即按输入不完整处理，不得静默跳过）。发现违反项**不擅改冻结块**，逐项记入完工报告「提请老板确认清单」（含所在权要条/块、违反的规则编号、原文摘录）。**直写全文稿工作流不执行本项**（权要与全文同轮产出、自始不冻结，判据见该节适用边界）。
2. 除非用户要求改动，保留已审权要不变。
...
4. 在 `docs/全文稿.md` 中补全权要一稿未写的章节，必须按 `references/rules/implementation.md`「全文稿分块撰写法」（唯一出处）逐块写、逐块过自检清单，不得一次性生成全文长文；具体实施方式框架按 `implementation.md` L8-0 Sxx 框架同构规则执行。
5. 在说明书中解释每一条权要步骤。
6. 加入有益效果的技术原因。
...
12. 报告完工前，先由 `docx` 执行层做通用 DOCX 验证，再由 `patent` 按 `docx-template.md` G8-0（逐分节内容落位）、G8-0b（发明名称段六属性由 `verify_docx_skeleton.py` 机械校验、五个章节标题格式人工核对）、G8-1（骨架与可见性）逐项验收；验收通过后，把 `docs/全文稿.md` 复制为 `docs/history/全文1稿.md`（快照留档，命名见 G1）；完工报告按“完工报告”的**校验清单**格式逐规则打 ✅/❌/➖。
...
1. 先按 `rules.md` 阶段读取表「直写全文稿」行读取 `global.md`、`claims-requirements.md`、`claims-field-background.md`（**全量，不冻结**）、`abstract-figures.md`、`invention-content.md`、`implementation.md`、`figures.md`、`docx-template.md`、`scoring.md`。确认案件文件夹下已有 `docs/` 子目录（无则创建），把交底书 DOCX 放入 `docs/`。先调用官方 `document-skills:docx`（即 docx）skill 满足执行层门槛（**不用其向上下文回读交底书正文**），随后直接执行 `scripts/disclosure_docx_to_md.py --input <案件文件夹>/docs/交底书.docx --output <案件文件夹>/docs/交底书.md` 完成确定性转换。权要规则不冻结，是本阶段必读核心。
...
3. **md 层先写权要稿**（`docs/权要稿.md`）：按 `claims-requirements.md`、`claims-field-background.md` 规则写权利要求书 → 技术领域 → 背景技术三部分（同权要一稿 step 5/6）。此时不写任何说明书章节、不动 Word。
...
6. **md 层续写全文稿**（`docs/全文稿.md`）：按 `implementation.md`「全文稿分块撰写法」（唯一出处）逐块撰写 L4 说明书摘要 → L5 摘要附图 → L6 发明内容 → L7 附图说明 → L8 具体实施方式（L8 按 L8-0 Sxx 框架同构、逐条主步骤分批写），逐块过自检清单、自检通过才写下一块；以已通过自检的权要三章为术语与链条基准。L9 附图见 step 7。
...
- `scripts/check_hard_rules.py --md <md 草稿> --stage claims-draft|full-draft [--claims-md <权要稿.md>] [--invention-name "<正式题名全称>"] [--draft-round 1|2|3|4] --json`：第一类硬规则机械检查（字数、断行、编号、禁用措辞等；full-draft 传 `--claims-md` 以豁免权要原文复述中的量词、并作为 L8-1 稿次字数区间的合并统计输入；**full-draft 必传 `--invention-name`**——本案正式发明题名全称含"及系统／及装置"后缀，供 G8-0b A 类槽位逐字校验，不传则该项静默不执行、只回一条配置缺失 suspect；**full-draft 必传 `--draft-round`**——本轮产出第几稿就传几，供 L8-1 稿次字数区间校验（1稿 15000–17000 双向区间、2/3/4稿 ≤20000，Word「字数」口径、`全文稿.md`＋`权要稿.md` 合并计），不传则该项记为未执行、回 `S-L8-1-draft-round-missing` suspect），闸门用法见权要一稿 step 7 / 全文一稿 step 8 / 返修 step 4。
- `scripts/extract_structure.py --md <md 草稿> --stage claims-draft|full-draft`：按标准写法（基准案 H2606029／H2605066；此处 A/B 指两份标杆案例，**与已废止的 L8「B 范式」无关**）抽取结构 JSON（权要分句、Sx 主步骤、子步骤、附图清单、依附关系），写法不规范时报错停。
...
- `scripts/verify_claims_alignment.py --md <全文稿.md> --claims-md <权要稿.md> --stage full-draft`：反向特征差集（L8-0，框架句实体名词 − 权要，suspect）+ 权要内撞名（`claims-requirements.md` L1-1，suspect）+ 步骤集差集（L8-1，权要分句产物名在三节缺失=hard FAIL）。`hard` 与 auditor 冲突以脚本为准；`suspect` 回传各路 auditor 复核。exit=hard 违规数。
...
- **校验清单**（权要一稿/全文一稿交付时必附）：**直接从合并落盘的 `docs/审查报告-*.md` 与验收结论转录，逐规则编号打勾，不重新校验**。用 emoji 标记：✅ 通过、❌ 失败（注明原因）、➖ 不适用（注明理由）。按块分组列出，格式如下：
  ...
  【审查执行】
  ✅ 审查执行方式：Workflow 编排 / Agent 派发 / 主 agent 串行审查（写实际方式；降级须注明原因）
  ...
  ✅ L1-1 权 1 ≤400 字
  ✅ L1-1 从权依附关系两轨平权（2 级质量项；轨 A 逐句对应型全拉平与轨 B 分支＋局部链条型均合格，多元化门槛仅轨 B 适用，见 `claims-requirements.md` L1-1）
  ✅ L6-1 发明内容逐条权要覆盖
  ...
  【提请老板确认清单】（返修稿与全文稿交付必附；无项时写「无」）
  ⚠️ 〔位置：块/权要条/段〕｜〔涉及规则编号〕｜〔冲突或违反内容摘录〕｜〔已采取的处置〕
...
- 是否有未解决的需要用户澄清的事项
- **提请老板确认清单**（返修稿与全文稿交付必附；无项写「无」）：把"已按批注/亲笔执行、但与规则或可实施性存在冲突"以及"发现了但按规矩不擅改"的项集中列出，使冲突可见而不是被静默吞掉（对外可见通道，判据唯一出处 `rules.md`「冲突处理」决策树）。三类来源必须都过一遍：①**批注与 1 级硬规则或可实施性冲突**——默认执行批注，同时列此清单；②**冻结块只读体检**发现的违反项（`implementation.md`「阶段映射与权要冻结」节）——不擅改冻结块，逐项列出；③**审核人亲笔重写的文本与硬规则字面冲突**（`revision.md` A4-1-核心）——一字不改保留，列出。每项写全四段：位置（块/权要条/段）+ 涉及规则编号 + 冲突或违反内容摘录 + 已采取的处置（执行批注／保留亲笔／未改冻结块）。**任何一类有项而清单未列 = 未完成交付。**
