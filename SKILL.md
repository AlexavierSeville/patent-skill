...
- **校验清单**（权要一稿/全文一稿交付时必附）：**直接从合并落盘的 `docs/审查报告-*.md` 与验收结论转录，逐规则编号打勾，不重新校验**。用 emoji 标记：✅ 通过、❌ 失败（注明原因）、➖ 不适用（注明理由）。按块分组列出，格式如下：
  ...
  多轮回修的案子以**最终通过轮**的结论为准；auditor 增量复核中"沿用上轮"的项照常打 ✅ 并可标注"(沿用)"。返修交付时清单换为返修校验项（署名正确、老板批注条数不减、老板 delText 字数不变、修订数符合预期、docx validate 无新增错误、**返修清单销项表 N/N 全核销**、**镜像块同步检查**（摘要/发明内容对应段/S 复述句/附图说明/"综上所述"段）、**注入后通读与反向保全 diff 已执行**）。
