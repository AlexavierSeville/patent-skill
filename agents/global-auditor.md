...
| `short_block_rules_path` | 短块规则文件的绝对路径,由主 agent 按阶段指定,**不整篇 Read**:`claims-draft` 传 `claims-requirements.md`,用 `sed -n '/^## L2\. /,/^## 权要阶段自检重点/p' <short_block_rules_path>` 提取(只执行 L2 + L3 节);`full-draft` 传 `implementation.md`,用 `sed -n '/^## L4\. /,/^## L6\. /p' <short_block_rules_path>` 与 `sed -n '/^## L7\. /,/^## L8\. /p' <short_block_rules_path>` 提取(只执行 L4 + L5 + L7 节)。 |
