...
| `content_rules_path` | `references/rules/implementation.md` 的绝对路径。**不整篇 Read**,用 `sed -n '/^## L6\. /,/^## L7\. /p' <content_rules_path>` 提取 L6 节(L6-1 / L6-2 / L6-3),再用 `sed -n '/^### L8-0 /,/^### L8-1 /p' <content_rules_path>` 提取 L8-0 反向断言条(发明内容/有益效果的反向实体名词差集判定依据),**只执行这两段**,其余节不在本路范围。 |
