...
| `content_rules_path` | `references/rules/invention-content.md` 的绝对路径。**不整篇 Read**,用 `sed -n '/^## L6\. /,/^<!-- EOF sentinel: invention-content -->/p' <content_rules_path>` 提取 L6 节(L6-1 / L6-2 / L6-3),再用 `sed -n '/^### L8-0 /,/^### L8-1 /p' <implementation.md>` 提取 L8-0 反向断言条(发明内容/有益效果的反向实体名词差集判定依据),**只执行这两段**,其余节不在本路范围。 |
...
7. **L6-2 质量项**:有益效果与技术特征逐项对应(光杆泛词字面命中已由 `check_hard_rules.py` 规则 25 机械终判,本路判"逐项对应"的语义完整性)。
...
9. **有益效果首项对应背景核心问题**(L6-1,W07):判定有益效果第（1）项所述效果是否与背景技术第三段"导致〔单一技术问题〕"的收口构成正面对应关系。把次要效果、通用效果或实现细节效果置于首项即 FAIL,须指出应提到首项的是哪一项及其理由。分项编号格式(全角括号数字、禁"其一/其二")已由 `check_hard_rules.py` 规则 22 机械终判,本路沿用不重跑,只判**排序的语义正确性**。
10. **第二方面系统段 Hard 沿用 + 简称线索处置**(L6-1,W05):`check_hard_rules.py` 规则 30 已对第二方面系统复述段的计算机设备式骨架与"的步骤"作确定性终判,本路**只沿用不重判**(引用其规则编号、位置与证据并纳入最短回修清单)。另须逐条处置 `mechanical_check_result.suspect_manifest.content` 中 `suspect_id == "S-W05-aspect-name"` 的线索——判定是否确为"第一方面所述方法"式简称,确认违规时给出应写的发明名称方法全称;若该处已用全称则给出证据豁免。漏处置视为本路未检查。
