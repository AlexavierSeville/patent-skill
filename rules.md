...
- **规则层容量治理（原则；执行判据的唯一出处是 `revision.md`「规则节清单容量上限」与「升格前去重检索」）**：①**同节上限**——同一规则节下由审批意见升格而来的具名清单条（条首标 W 编号者）上限 8 条，达限后只许合并同类项，不许新增第 9 个编号；②**跨节去重**——升格前必须对 `references/rules/*.md` 全量语义检索，已被他节覆盖的判“已覆盖”并指向该节编号，不在本节另立编号（与上文「单一出处原则」同源：同一义务只能有一个编号）。**reviewer 审 PR 依据**：凡新增规则编号的 PR，说明中须给出去重检索命令与结论、以及该节升格条计数（新增后仍 ≤8）；二者缺一即按“又加一条编号”式沉淀否决。升格准入判定本身（三进四不进）与决策留痕见 `revision.md` 学习闭环，本索引不复述判据。
