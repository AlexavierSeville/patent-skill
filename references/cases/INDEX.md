...
- **撰写者经验档案** → `experience/<撰写者>.md`（xxb/wpy/lxl 一人一档，含对应审稿人偏好+升格状态；**不在任何撰写/返修阶段默认读取**——可迁移条目均已升格进 `references/rules/`，日常按规则文件执行即可，仅整理 skill／沉淀经验／用户要求调规则时才读；读取纪律唯一出处 `rules.md`「经验档案读取纪律」。模板与入档纪律见 `experience/README.md`）
